...
export default function OnScreenKeyboard({ isOpen, value, onChange, onClose }: OnScreenKeyboardProps) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 bg-[#141414] border-t border-[#222] p-4">
      {/* Input display */}
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        <div className="flex gap-1.5">
          {rows[0].map(key => (
            <button key={key} onClick={() => handleKey(key)}
              className="w-[72px] h-[52px] bg-[#2a2a2a] rounded-lg text-white text-lg font-medium hover:bg-[#333] transition-colors relative">
        ...
        <div className="flex gap-1.5 ml-8">
          {rows[1].map(key => (
            <button key={key} onClick={() => handleKey(key)}
              className="w-[72px] h-[52px] bg-[#2a2a2a] rounded-lg text-white text-lg font-medium hover:bg-[#333] transition-colors">
              {shifted ? key.toUpperCase() : key}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-1.5">
          <button onClick={() => setShifted(!shifted)}
            className={`w-[72px] h-[52px] rounded-lg text-white text-lg hover:bg-[#333] transition-colors ${shifted ? 'bg-[#333]' : 'bg-[#1a1a1a]'}`}>
            ⇧
          </button>
          {rows[2].map(key => (
            <button key={key} onClick={() => handleKey(key)}
        ...
        {/* Row 4 - Space bar */}
        <div className="flex gap-1.5">
          <button className="w-[72px] h-[52px] bg-[#1a1a1a] rounded-lg text-white text-sm hover:bg-[#333] transition-colors">
            ?123
          </button>
          <button onClick={() => handleKey(',')}
            className="w-[52px] h-[52px] bg-[#2a2a2a] rounded-lg text-white text-lg hover:bg-[#333] transition-colors">
            ,
          </button>
          <button onClick={() => handleKey(' ')}
            className="w-[360px] h-[52px] bg-[#2a2a2a] rounded-lg text-white hover:bg-[#333] transition-colors" />
          <button className="w-[52px] h-[52px] bg-[#2a2a2a] rounded-lg text-white text-lg hover:bg-[#333] transition-colors">
            😊
          </button>
          <button onClick={() => handleKey('.')}
            className="w-[52px] h-[52px] bg-[#2a2a2a] rounded-lg text-white text-lg hover:bg-[#333] transition-colors">
